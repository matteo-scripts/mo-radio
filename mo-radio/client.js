...
for (let i = 0, length = GetNumResourceMetadata("mo-radio", "supersede_radio"); i < length; i++) {
    const radio = GetResourceMetadata("mo-radio", "supersede_radio", i);
    ...
    try {
        const data = JSON.parse(GetResourceMetadata("mo-radio", "supersede_radio_extra", i));
        if (data !== null) {
            customRadios.push({
                "isPlaying": false,
                "name": radio,
...
RegisterNuiCallbackType("mo-radio:ready");
on("__cfx_nui:mo-radio:ready", (data, cb) => {
